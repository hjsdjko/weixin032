<template>
	<view class="content">
		<view :style='{"width":"100%","padding":"0 0 120rpx","position":"relative","background":"#f4f4f4","height":"auto"}'>
			<view v-if="user&&user.id" :style='{"padding":"0 24rpx 0 24rpx","margin":"0px 0 20rpx 0","borderRadius":"0 0 60rpx 60rpx","background":"#1D5DCA","display":"flex","width":"100%","height":"280rpx"}' class="header" v-bind:class="{'status':isH5Plus}" @tap="onPageTap2('../user-info/user-info',1)">
				<view :style='{"width":"calc(100% - 112rpx)","alignItems":"center","display":"flex","height":"100%"}' v-if="tableName=='xuesheng'" class="userinfo">
					...
					<view :style='{"width":"100%","flex":"1","flexDirection":"column","justifyContent":"center","display":"flex","height":"100%"}' class="info">
						<view :style='{"width":"100%","lineHeight":"36rpx","fontSize":"28rpx","color":"#ffffff"}'>{{user.xuehao}}<text v-if="user.vip&& user.vip=='是'">(VIP)</text></view>
					</view>
				</view>
				<view :style='{"width":"calc(100% - 112rpx)","alignItems":"center","display":"flex","height":"100%"}' v-if="tableName=='jiaoshi'" class="userinfo">
					<image :style='{"width":"108rpx","padding":"0 0 0 0","margin":"0 20rpx 0 0","borderRadius":"100%","height":"108rpx"}' :src="user.touxiang?baseUrl+user.touxiang:'/static/gen/upload.png'"></image>
					<view :style='{"width":"100%","flex":"1","flexDirection":"column","justifyContent":"center","display":"flex","height":"100%"}' class="info">
						<view :style='{"width":"100%","lineHeight":"36rpx","fontSize":"28rpx","color":"#ffffff"}'>{{user.jiaoshigonghao}}<text v-if="user.vip&& user.vip=='是'">(VIP)</text></view>
					</view>
				</view>
				<view :style='{"border":"0px solid #eeeeee","width":"72rpx","alignItems":"center","justifyContent":"center","display":"flex","height":"100%"}' class="setting">
					<view :style='{"border":"0px solid #eeeeee","width":"72rpx","lineHeight":"72rpx","fontSize":"72rpx","color":"#ffffff","borderRadius":"0px"}' class="iconfont icon-shezhi1"></view>
				</view>
			</view>
			<view v-else :style='{"padding":"0 24rpx 0 24rpx","margin":"0px 0 20rpx 0","borderRadius":"0 0 60rpx 60rpx","background":"#1D5DCA","display":"flex","width":"100%","height":"280rpx"}' class="header" v-bind:class="{'status':isH5Plus}" @tap="onPageTap2('../user-info/user-info',1)">
			...
			<view :style='{"padding":"0px 20rpx 40rpx 20rpx","margin":"40rpx 40rpx 120rpx 40rpx","borderRadius":"8rpx","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"calc(100% - 80rpx)","justifyContent":"space-between","height":"auto"}' class="list">
				<block v-for="(item,index) in menuList" v-bind:key="item.roleName" v-if="item.child.length&&item.child[0].tableName!='orders'">
					<block v-if="item.child.length>1">
						<view @tap="secondClick(index)" :style='{"border":"0px solid #6fcbcb60","padding":"0px 0px","margin":"0 0 0px 0","borderColor":"#eeeeee","alignItems":"center","textAlign":"center","display":"flex","transition":"all 0s","borderRadius":"0px","flexWrap":"wrap","background":"#ffffff","borderWidth":"0 0 2rpx 0","width":"100%","position":"relative","borderStyle":"solid","height":"92rpx"}'
				...
				<view v-if="user&&user.id" @tap="passwordShow()" :style='{"border":"0px solid #6fcbcb60","padding":"0px 0px","margin":"0 0 0px 0","borderColor":"#eeeeee","alignItems":"center","textAlign":"center","display":"flex","transition":"all 0s","borderRadius":"0px","flexWrap":"wrap","background":"#ffffff","borderWidth":"0 0 2rpx 0","width":"100%","position":"relative","borderStyle":"solid","height":"92rpx"}' class="li" hover-class="hover">
